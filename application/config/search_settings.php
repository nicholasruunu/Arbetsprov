<?php
	...
	$config['search_engines'] = array(
		'google' => array(
			...
			'regex' => '/<h3\sclass="?r"?><a href="'.
				'(?P<url>http:\/\/[^"]+?)".*?>'.
				'(?P<title>[^"\n\r\t]+?)<\/a><\/h3>(?:<span\sclass="?std\snobr"?>.*?<\/span>)?(?:<div\sclass="?s"?>(<div\sclass="?f"?>.*?<\/div>)?'.
				'(?P<text>.*?)(<br>.*?)?)?<span\sclass="?f"?>/m'
		),
		'bing' => array(
			'url' => 'http://www.bing.com/search?q=',
			'regex' => '/<div\sclass="?sb_tlst"?><h3><a.+?href="'.
				'(?P<url>http:\/\/[^"]+?)".*?>'.
				'(?P<title>[^\n\r\t]+?)<\/a>.*?(?:<p>'.
				'(?P<text>.*?)<\/p>)?<div\sclass="sb_meta">/m'
		),
		'yahoo' => array(
			'url' => 'http://search.yahoo.com/search?n=20&p=',
			'regex' => '/<div\sclass="?res"?><div>(?:\s<span\sclass="?ext"?>.*?<\/span>\s)?<h3><a.+?href="'.
				'(?P<url>http:\/\/[^"]+?)".*?>'.
				'(?P<title>[^\n\r\t]+?)<\/a>.*?(?:<div\sclass="?(sm-abs|abstr)"?>'.
				'(?P<text>.*?)<\/div>.*?)?<span\sclass="?url"?>/m'
		),
	);